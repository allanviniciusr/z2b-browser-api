...
def get_llm_model(provider: str, **kwargs):
    ...
    if provider == "anthropic":
        ...
        # Adiciona headers específicos para OpenRouter se estiver usando seu endpoint
        default_headers = {}
        if "openrouter.ai" in base_url:
            default_headers = {
                "HTTP-Referer": os.getenv("HTTP_REFERER", "http://localhost:7788"),
                "X-Title": os.getenv("X_TITLE", "ZAP2B Browser Agent"),
                "HTTP-OR-APP-ID": os.getenv("HTTP_HEADER_OR_APP_ID", "zap2b-browser-agent-v1")
            }
